...
router_auth = APIRouter( tag= ['Auth'] )
logging.basicConfig(level=logging.INFO)
_models.Base.metadata.create_all(_models.engine)

@router_auth.post("/api/users")
...
@router_auth.get("/api/users/me", response_model=_schemas.User)
async def get_user(user: _schemas.User = _fastapi.Depends(_services.get_current_user)):
    return user


@router_auth.get("/api/users/profile")
...
@router_auth.post("/api/users/generate_otp", response_model=str)
...
@router_auth.post("/api/users/verify_otp")
