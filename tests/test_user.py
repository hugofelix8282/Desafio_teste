...
def test_criar_usuario_sucesso(client):
    response = client.post("/auth/v1/registro", json={
        "nome": "João Valido",
...
def test_login_usuario_sucesso(client):
    ...
    client.post("/auth/v1/registro", json={
        "nome": "Maria",
        "email": "maria@example.com",
        "password": "senhateste"
    })

    response = client.post("/auth/v1/login", data={
...
@pytest.mark.asyncio
def test_login_usuario_nao_existe(client):
    response = client.post("/auth/v1/login", data={
